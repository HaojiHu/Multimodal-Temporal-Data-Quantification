...
import warnings
import math
from utilsforecast.processing import is_nan
...
for Store in range(1, 100):
    ...
    # print(df_new)
    df_covariate = df_covariate.fillna(0)
    ...
    # for v in [ 'snap_TX']:
        ...
        X_train = train.drop(columns=left+discared_columns)
        # print(X_train)
        for lag in lag_days:
            X_train[f"lag_{lag}"] = df[:train_end][f"lag_{lag}"]
        y_train = df[:train_end][Store]

        X_test = test.drop(columns=left + discared_columns)
        ...
        # def remove_Nan(y_test, y_pred):
        #     removed_test = []
        #     removed_pred = []
        #     for i, pred in enumerate(y_pred):
        #         if math.isnan(pred) == False:
        #             removed_test.append(pred)
        #             removed_pred.append(y_test[i])
        #
        #     return removed_test, removed_pred
        #
        #
        # removed_test, removed_pred = remove_Nan(y_test.to_list(), y_pred)
