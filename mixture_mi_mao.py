...
def mixture_mi(disc, cont):
    ...
    if len(dis_dis) == 0:
        ...
        p_a = (len(dis_dis[0])-distinct_repeated_values) / len(disc)
...
def NMI(discs, conts):
    ...
    cont = entropy_estimate_1d_continuous(conts, islog2=True) + np.log2(scale)
    ...
    # return mi / max(disc, cont)
    # return mi / cont
    return math.sqrt(1 - math.exp((-2*mi)/(np.log(len(discs)))))
